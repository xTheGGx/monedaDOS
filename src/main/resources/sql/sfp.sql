...
CREATE TABLE CUENTA (
                        ...
                        usuario_id INT NOT NULL,             -- dueño de la cuenta
                        nombre VARCHAR(100) NOT NULL,
                        tipo ENUM('EFECTIVO','DEBITO','CREDITO','OTRO') NOT NULL,
                        saldo DECIMAL(15,2) NOT NULL DEFAULT 0.00,
                        moneda CHAR(3) NOT NULL,             -- ISO 4217 (ej. MXN, USD)
...
CREATE TABLE CATEGORIA (
                           ...
                           usuario_id INT NULL, -- NULL => global/por defecto
                           nombre VARCHAR(80) NOT NULL,
                           tipo ENUM('INGRESO','EGRESO') NOT NULL,
                           clasificacion ENUM('NECESIDAD','DESEO','AHORRO') NULL, -- solo aplica a EGRESO
                           PRIMARY KEY (id_categoria),
                           CONSTRAINT fk_cat_usuario FOREIGN KEY (usuario_id) REFERENCES USUARIO(id_usuario)
...
CREATE TABLE TRANSACCION (
                             ...
                             usuario_id INT NOT NULL, -- quien registra (útil en cuentas compartidas)
                             ...
                             CONSTRAINT fk_tx_usuario FOREIGN KEY (usuario_id) REFERENCES USUARIO(id_usuario)
                                 ON UPDATE CASCADE ON DELETE RESTRICT,
                             CONSTRAINT fk_tx_cuenta FOREIGN KEY (cuenta_id) REFERENCES CUENTA(id_cuenta)
                                 ON UPDATE CASCADE ON DELETE RESTRICT,
                             CONSTRAINT fk_tx_categoria FOREIGN KEY (categoria_id) REFERENCES CATEGORIA(id_categoria)
                                 ON UPDATE CASCADE ON DELETE RESTRICT,
...
CREATE TABLE META_AHORRO (
                             ...
                             PRIMARY KEY (id_meta),
                             CONSTRAINT fk_meta_usuario FOREIGN KEY (usuario_id) REFERENCES USUARIO(id_usuario)
                                 ON UPDATE CASCADE ON DELETE CASCADE,
                             CONSTRAINT fk_meta_cuenta FOREIGN KEY (cuenta_id) REFERENCES CUENTA(id_cuenta)
                                 ON UPDATE CASCADE ON DELETE SET NULL,
                             CONSTRAINT chk_meta_montos CHECK (monto_actual <= monto_objetivo AND monto_objetivo > 0)
) ;
...
CREATE TABLE ALERTA (
                        ...
                        usuario_id INT NOT NULL,
                        fecha DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                        tipo ENUM('PRESUPUESTO','TARJETA','OTRO') NOT NULL DEFAULT 'OTRO',
                        mensaje VARCHAR(255) NOT NULL,
                        leido BOOLEAN NOT NULL DEFAULT FALSE,
                        PRIMARY KEY (id_notificacion),
                        CONSTRAINT fk_alerta_usuario FOREIGN KEY (usuario_id) REFERENCES USUARIO(id_usuario)
                            ON UPDATE CASCADE ON DELETE CASCADE,
                        INDEX idx_alerta_usuario_leido (usuario_id, leido)
) ;
...
CREATE TABLE REPORTE (
                         ...
                         descripcion_reporte VARCHAR(255) NULL,
...
BEGIN
    ...
    IF v_tipo = 'EGRESO' AND NEW.monto >= 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Egreso debe registrarse con monto negativo';
  ELSEIF v_tipo = 'INGRESO' AND NEW.monto <= 0 THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Ingreso debe registrarse con monto positivo';
END IF;
END$$
...
-- Actualiza saldo de la cuenta al insertar/actualizar/eliminar
...
BEGIN
    ...
    BEGIN
        ...
        UPDATE CUENTA SET saldo = saldo + (NEW.monto - OLD.monto)
...
-- =========================================================
        CREATE INDEX idx_cat_usuario_tipo ON CATEGORIA(usuario_id, tipo);
        CREATE INDEX idx_tx_fecha ON TRANSACCION(fecha);

-- =========================================================
--  VISTAS PARA DASHBOARD (opcionales)
-- =========================================================

-- Ingresos vs Egresos por mes (por usuario)
        ...
        GROUP BY t.usuario_id, DATE_FORMAT(t.fecha, '%Y-%m-01');

-- Distribución 50/30/20 (egresos por macro-categoría) por mes
...
-- Categorías globales por defecto (usuario_id = NULL)
        ...
        -- (Opcional) Usuario demo y su configuración 50/30/20
-- NOTA: contrasena_hash debe ser un hash real (ej. BCrypt). Aquí es placeholder.
        INSERT INTO USUARIO (nombre, email, contrasena_hash, id_rol)
        VALUES ('Demo', 'demo@ejemplo.com', '$2y$12$HASH_DE_EJEMPLO_NO_REAL', 2);
...
-- (Opcional) Cuenta demo en MXN y asignación al usuario demo
        INSERT INTO CUENTA (nombre, tipo, saldo, moneda) VALUES ('Efectivo MXN', 'EFECTIVO', 0.00, 'MXN');
        INSERT INTO USUARIO_CUENTA (id_usuario, id_cuenta, es_administrador)
        SELECT u.id_usuario, c.id_cuenta, TRUE
        FROM USUARIO u, CUENTA c
        WHERE u.email='demo@ejemplo.com' AND c.nombre='Efectivo MXN';
